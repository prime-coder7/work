...
$seller_id = (string) $_SESSION['seller_id'];
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['reply_message'], $_POST['message_id'])) {
    ...
    if (!empty($reply_message) && $message_id > 0) {
        ...
        if ($original_message) {
            $receiver_id = (string) $original_message['sender_id']; // User ID

            // Insert reply into message table
            $insert_reply = $conn->prepare("INSERT INTO message (sender_id, receiver_id, message, reply_to) VALUES (?, ?, ?, ?)");
            $insert_reply->execute([$seller_id, $receiver_id, $reply_message, $message_id]);

            header("Location: ../admin_pannel/admin_message.php");
            exit();
        }
    }
}
...
$select_messages = $conn->prepare("
    SELECT m.id, m.message, m.sender_id, m.receiver_id,
    ...
    ORDER BY m.id DESC
");
$select_messages->execute([$seller_id, $seller_id]);
...
<body>
    <header>
        <a href="../admin_pannel/dashboard.php" class="back-icon"><i class="fa fa-home"></i></a>
        Admin Messages
    </header>
    ...
    <div class="container mt-4">
        ...
        <?php foreach ($messages as $message): ?>
            <div class="message-box <?= $message['sender_id'] == $seller_id ? 'admin' : '' ?>">
